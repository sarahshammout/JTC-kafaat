    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="flex items-center gap-2 px-4 sm:px-5 py-4 border-b border-slate-100">
          ...
          <span className="font-semibold text-slate-700 text-sm sm:text-base">Total: {donations.length}</span>
        </div>
              ...
              {donations.length === 0
                ...
                : donations.map(d => (
                  <tr key={d.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <Td cls="max-w-[140px] sm:max-w-[160px]"><span className="truncate block text-slate-400 text-xs">{d.notes || "—"}</span></Td>
                    <Td>{d.submitted}</Td>
                  </tr>
                ))}
            </tbody>
