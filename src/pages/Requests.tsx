    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-100">
          ...
          <span className="font-semibold text-slate-700">Total: {requests.length}</span>
        </div>
              ...
              {requests.length === 0
                ...
                : requests.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <Td>{r.gender}</Td>
                    <Td>{r.city}</Td>
                    <Td cls="max-w-[200px]"><span className="text-slate-400 text-xs truncate block">{r.reason}</span></Td>
                    <Td>{r.submitted}</Td>
                  </tr>
                ))}
            </tbody>
